function final_image = myCLAHE(old_image_path, N, thresh, save_path)
    figure
    hold on
    ...
    for d=1:dim
        ...
        for j=1:old_cols
            ...
            final_image(1,j,d)=HE(stack,thresh);
            for i=2:old_rows
               stack=stack(2:N,:);
               stack=[stack; new_image(i+N-1,j:j+N-1)];
               final_image(i,j,d)=HE(stack,thresh);
            end
        end
...
function final=HE(old_image,thresh)
    ...
    if (max(stack)/sum(stack))>thresh
        excess=0;
        limit=thresh*sum(stack);
        for i=1:256
            excess=excess+max(0,stack(i)-limit);
        end
        
        unravel_image=min(unravel_image,limit);
        
        unravel_image=unravel_image+(excess/sum(stack));
